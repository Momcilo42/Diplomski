...
int main( int argc, char** argv )
{
	for(int i = 1; i < argc; i++)
	{
		...
		if(!imageLocation.compare("-test")) testAll(&digit1, &digit2, &digit3);
		else
		{
			...
			if(!digit1.empty()) imshow("digit1 "+imageLocation, digit1);
			if(!digit2.empty()) imshow("digit2 "+imageLocation, digit2);
			if(!digit3.empty()) imshow("digit3 "+imageLocation, digit3);
		}
